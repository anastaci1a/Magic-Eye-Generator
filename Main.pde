void setup() {
  /*
    ...
    > CONFIG.TEXTURE_FILE_SCALE (4): (Only applicable when CONFIG.TEXTURE_MODE is set to TEXTURE_MODE.FILE)
      - scale factor for the resolution of the inputted texture image(s)
        (i.e. "2x2", defaults to autoscaling if not set)
    ...
    > CONFIG.TEXTURE_ALGORITHM (5): (Only applicable when CONFIG.TEXTURE_MODE is set to TEXTURE_MODE.ALGORITHM)
      - algorithm of your texture(s) to be generated
        ...
        - algorithm options:
          ...
          > ALGORITHMS.PERLIN_NOISE_GRAYSCALE (2):
            - this algorithm generates grayscale images using perlin noise
          > ALGORITHMS.PERLIN_NOISE_COLOR (3):
            - this algorithm generates full color images using perlin noise
      - note: all algorithms regenerate a new random texture every frame
    
    > CONFIG.TEXTURE_ALGORITHM_RESOLUTION (6): (Only applicable when CONFIG.TEXTURE_MODE is set to TEXTURE_MODE.ALGORITHM)
      - resolution of each texture generated by the selected algorithm
    ...
    > CONFIG.ANIM_LOOPS (7):
      - amount of times the depth map frames loop (if your depth map has only one frame, you can use this determine the amount of frames for that single depth map image)
        (i.e. 5, defaults to 1 (must be integer))
    
    > CONFIG.ANIM_TEXTURE_MOVEMENT (8):
      - amount of times background texture shifts its position horizontally and vertically by full width and height increments, creates a moving background effect
